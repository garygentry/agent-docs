#!/usr/bin/env node
// check-docs.mjs — docs drift guard for packages/docs
...
// Manifest-validity: slug uniqueness (00 §2.2 rule 5). JSON Schema can't express
// "unique slug property across array items", so the guard owns it. A duplicate
// slug is a MANIFEST ERROR (not page drift): report it and exit 2 — the same
// class as a missing/invalid manifest — distinct from exit 1 (drift findings).
function assertUniqueSlugs() {
  ...
  for (const [slug, n] of dupes) {
    console.error(`check-docs: [duplicate-slug] docs.manifest.json — slug '${slug}' appears ${n} times`);
  }
  process.exit(2);
}
assertUniqueSlugs();
...
// §4.1b (#24/#29) — Rule 1b: non-canonical (base-unsafe) internal links. A
// relative internal link (`foo/bar/`, `./x`, `../y`) — or any link to a
// `.md`/`.mdx` file — renders verbatim on deploy. rehype-base-links.mjs only
// rewrites relative `.md` links and base-prefixes root-absolute `/slug/` links,
// so a bare relative route still 404s under a subpath base. Require the
// root-absolute `/slug/` form for internal links authored natively for the site.
