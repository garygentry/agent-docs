...
The concrete contracts `doc-site` works against: the token vocabulary, the
...
provenance shape, and the shared deploy env contract. All of these are _implemented_
in `skills/doc-site/` and validated by `src/test/doc-site-*.test.ts`.
...
5. `slug` values unique across `pages`. **Note:** JSON Schema cannot express per-slug
   ...
   drift guard**, not the static schema — the schema legitimately _accepts_ a
...
`{{DOCS_PKG_DIR}}/docs.drift.rules.mjs` _iff present_. It `export default`s an array of
