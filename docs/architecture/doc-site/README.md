...
`doc-site` is an **agent skill**, not a CLI — an engineer invokes it through
their coding agent ("set up a docs site for this repo"). The agent then runs the
seven-phase procedure in `skills/doc-site/SKILL.md`:
...
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `skills/doc-site/SKILL.md`                             | The lean 7-phase procedure + the canonical 17-token substitution table.                                                                              |
| `skills/doc-site/references/*.md`                      | Per-phase / per-component agent references (detect, interview, core, manifest-schema, symlink, diagrams, deploy-\*, monorepo, drift-guard, rerun).   |
| `skills/doc-site/references/docs.manifest.schema.json` | The static JSON Schema (Draft 2020-12) copied verbatim into the target repo to validate its manifest.                                                |
| `skills/doc-site/references/templates/<group>/*.tmpl`  | The byte-identical template assets, grouped by component (`core/`, `symlink/`, `diagrams/`, `deploy/*`, `monorepo/`, `drift-guard/`).                |
| `tools.manifest.json`                                  | Registers the skill (`{ name, type:"skill", source:"skills/doc-site", description }`) so the build emits it to all 5 targets.                        |
...
- **You want it to deploy or host the site.** It emits deploy _config_ (GitHub Pages
