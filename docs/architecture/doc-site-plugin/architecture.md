...
How `doc-site-plugin` turns an agent-led interview into a working Astro + Starlight
...
skill (`skills/doc-site-plugin/`), cross-referenced to the design specs under
...
the *tests* (`src/test/doc-site-*.test.ts`) and one line of `SAMPLE_RELPATHS` for the
...
The **load-bearing principle**: detection ambiguity is *never* a hard-fail. Every
...
- **No in-template logic.** Where behavior differs by toolchain (Bun+pnpm vs Node+npm),
  the template ships *named fragments* and the agent *selects* one — it never evaluates
...
*generated*, never hand-kept, it cannot drift from the manifest.
...
re-run, and each managed file the run would emit gets a per-file decision *before*
writing:
...
- **No external transmission** — detection reads only local files; the *only* network
  use is `npm view` version resolution and dependency install, which send package
  *names*, never repo contents (`REQ-SEC-01..03`).
